...
namespace SailTrim
{
    ...
    internal static class MapHud
    {
        ...
        private static void Build()
        {
            ...
            _panel.sizeDelta = new Vector2(360f, 96f);
            _home = new Vector2(220f, 130f);          // bottom left, clear of the map's own legend
            _panel.anchoredPosition = _home;

            TMP_Text src = hud.m_healthText;
            _title = Line("Title", src, 22f, new Vector2(0f, 34f));
            _line1 = Line("Line1", src, 18f, new Vector2(0f, 10f));
            _line2 = Line("Line2", src, 18f, new Vector2(0f, -10f));
            _line3 = Line("Line3", src, 18f, new Vector2(0f, -30f));
            Plugin.Log.LogInfo("SailTrim: map readout built");
        }

        private static TMP_Text Line(string name, TMP_Text src, float size, Vector2 pos)
        {
            ...
            rt.sizeDelta = new Vector2(360f, 22f);
        ...
        private static void Write(Ship ship, SailTrimShip st)
        {
            ...
            {
                _line3.text = $"set {Mathf.Abs(off):0} deg to {(off > 0f ? "starboard" : "port")}, drift {sideways:0.0} kn";
